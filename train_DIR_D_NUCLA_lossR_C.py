...
def test(dl, net, epoch):
    ...
    with torch.no_grad():
        ...
        for _, sample in enumerate(dl):
            ...
            l1_C_B_t.append((C*B).abs().mean().detach().cpu())
...
def main(args):
    ...
    for epoch in range(1, args.Epoch+1):
        ...
        for _, sample in enumerate(trainloader):
            ...
            ML1_C = C.abs().mean()
            ML1_B = B.abs().mean()
            loss = args.lam2 * MSE_C + args.Alpha_C * ML1_C
            
            loss.backward()
            # torch.nn.utils.clip_grad_norm_(net.parameters(), args.bs)
            ...
            l1_C_B.append((C*B).abs().mean().detach().cpu())
